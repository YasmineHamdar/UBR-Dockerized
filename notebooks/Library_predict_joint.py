...
input_path = "/home/yhh05/smogN/Daily_minus_joint_manual.csv"
input_path_test = "/home/yhh05/smogN/joint_manual_cleaned.csv"
...
#                 'Elevation(m)_1','Elevation(m)_2','Elevation(m)_3','Elevation(m)_4',
#                 'Elevation(m)_5','Elevation(m)_6', 'NETRAD', 'LE_ebr_corr(mm)', 'ET_bowen_corr_mm'
#                 ]    
...
df.drop(columns=['Site', 'Month', 'Vegetation', 'Month_0', 'Vegetation_0', 'Site_0'], inplace=True)
print(df.columns)
...
#read csv
df_t = pd.read_csv(input_path_test, delimiter=',')
columnToDrop = ["Site", "Month", 'Date', 'Year']
# drop NaN values
...
#set output variable between 1 and 15 only
df_t = df_t[df_t[output_column].between(1, 15)]
df_t.rename(columns_rename, inplace=True)

#drop desired columns, rename, and drop the nans
df_t = df_t.drop(columnsToDrop, axis = 1)
df_t.dropna(inplace=True)

#generate lags for columns
lagsForColumns = ["WS", "RH", "TA", "Eeflux_LST", "Eeflux_Albedo", "Eeflux_NDVI"]
df_t = generate_lags(df_t, lagsForColumns)

df_t = binary_encode_column(df_t, "Site")
df_t = binary_encode_column(df_t, "Month")
df_t = binary_encode_column(df_t, "Vegetation")
df_t.drop(columns=['Site','Month', 'Vegetation', 'Month_0', 'Vegetation_0', 'Site_0'], inplace=True)
print(df_t.columns)
...
print(df_t.shape)
print("checking null values in the whole dataset")
print(df_t.isnull().values.any())

#df[output_column] = df['ET_bowen_corr_mm'] / df['ETo']
#df = df.drop(['ET_bowen_corr_mm', 'ETo'], axis=1)
df_t.dropna(inplace=True)
df_t.to_csv(output_path + 'eeflux_joint.csv')
df_t = df_t[df.columns]
print(df_t.shape)
print("FINAL SHAPE FOR JOINT")

########################################################################################################################
                                            #Train and Test
########################################################################################################################
X_train = df.drop([output_column], axis =1 )
X_test = df_t.drop([output_column], axis =1 )
y_train = pd.DataFrame()
y_test = pd.DataFrame()
y_train['LE_bowen_corr_mm'] = df[output_column]
y_test['LE_bowen_corr_mm'] = df_t[output_column]
...
#
df = X_comb
df['LE_bowen_corr_mm'] = y_comb

print(np.average(df['LE_bowen_corr_mm']), 'average target')
